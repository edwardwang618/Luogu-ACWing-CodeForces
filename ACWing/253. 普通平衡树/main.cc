...
int get_rank_by_key(int p, int key) {
  ...
  else if (tr[p].key < key) return tr[tr[p].l].sz + tr[p].cnt + get_rank_by_key(tr[p].r, key);
  else return tr[tr[p].l].sz + 1;
}

int get_key_by_rank(int p, int rank) {
  if (!p) return INF;
  if (tr[tr[p].l].sz >= rank) return get_key_by_rank(tr[p].l, rank);
  else if (tr[tr[p].l].sz + tr[p].cnt < rank) return get_key_by_rank(tr[p].r, rank - tr[tr[p].l].sz - tr[p].cnt);
